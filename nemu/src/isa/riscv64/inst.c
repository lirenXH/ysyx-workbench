...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, shamt = BITS(src2,5,0);R(dest) = src1 << shamt);
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, shamt = BITS(src2,4,0);R(dest) = SEXT(BITS((src1 << shamt),31,0),32));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, shamt = BITS(src2,5,0);R(dest) = src1 >> shamt);
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, shamt = BITS(src2,5,0);R(dest) = SEXT((BITS(src1,31,0) >> shamt),32));
  INSTPAT("0000000 ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^  src2);
  INSTPAT("0000000 ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 &  src2);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, if((unsigned)src1 < (unsigned)src2) R(dest) = 1; else R(dest) = 0);
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, shamt = BITS(src2,5,0),R(dest) = ((signed)src1 >> shamt));                      //doubt  suanshuyiwei
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, shamt = BITS(src2,5,0),R(dest) = SEXT((signed)(BITS(src1,31,0) >> shamt),32));//doubt  suanshuyiwei
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(dest) = SEXT((BITS(src1,31,0) >> BITS(src2,4,0)),32));               //doubt  suanshuyiwei
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, if((unsigned)src1 < (unsigned)src2) R(dest) = 1; else R(dest) = 0);                        //doubt
