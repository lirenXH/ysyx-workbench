...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((sword_t)src1 <  (sword_t)src2) s->dnpc = s->pc + dest);            //doubt  fuhao
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if((word_t)src1 < (word_t)src2) s->dnpc = s->pc + dest);
  INSTPAT("??????? ????? ????? ??? ????? 00100 11", addi   , I, R(dest) = src1 + src2);                                              //yes!
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = SEXT((word_t)(BITS(src1,31,0) >> BITS(src2,5,0)),32));
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^  src2);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 &  src2);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, if((word_t)src1 < (word_t)src2) R(dest) = 1; else R(dest) = 0);
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((sword_t)src1 >> BITS(src2,5,0)));                      //doubt  suanshuyiwei
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT((sword_t)BITS(src1,31,0) >> BITS(src2,5,0),32));    //doubt  suanshuyiwei
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, if((sword_t)src1 < (sword_t)src2) R(dest) = 1; else R(dest) = 0);
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(dest) = SEXT((sword_t)(BITS(src1,31,0) >> BITS(src2,4,0)),32));               //doubt  suanshuyiwei
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = SEXT((word_t)(BITS(src1,31,0) >> BITS(src2,4,0)),32)); 
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, if((word_t)src1 < (word_t)src2) R(dest) = 1; else R(dest) = 0);                        //doubt
