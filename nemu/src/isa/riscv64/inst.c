...
#define csr1C(n) do { *csr1 = C(n); } while (0)
...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, word_t *csr1, int type) {
  ...
  int cs1 = BITS(i, 31, 20);//这是个局部变量
  destR(rd);
  switch (type) {
    case TYPE_I: src1R(rs1); destR(rd); src2I(immI(i)); csr1C(cs1); break;
...
static int decode_exec(Decode *s) {
  word_t dest = 0, src1 = 0, src2 = 0,csr1 = 0;
  s->dnpc = s->snpc;
  word_t t; //中转变量
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, t = csr1;csr1=src1     ;R(dest)=t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, t = csr1;csr1=t | src1 ;R(dest)=t);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, t = csr1;csr1=t & ~src1;R(dest)=t);
  //INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, t = s->pc + (word_t)4;s->dnpc = (src1 + src2) & 0xfffffffe;R(dest) = t);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, isa_raise_intr(11,C(0x305)));//C(0x305) is mtvec 
