...
extern char _end;
intptr_t end = (intptr_t)&_end;         //program break一开始的位置位于_end
...
int _open(const char *path, int flags, mode_t mode) {              //打开一个文件, 并返回相应的文件描述符
  printf("path = %s\n", path);
  printf("(intptr_t)path = %x\n",(intptr_t)path);
  return _syscall_(SYS_open,(int)path,(intptr_t)flags,(intptr_t)mode);
}
...
void *_sbrk(intptr_t increment) {         //返回-1表示堆区调整失败
  intptr_t old = end;                     //记录旧program break的位置
  intptr_t new = end + increment;         //被调用时, 根据记录的program break位置和参数increment, 计算出新program break
  if(_syscall_(SYS_brk,new,0,0) == 0){    //通过SYS_brk系统调用来让操作系统设置新program break , 若SYS_brk系统调用成功, 该系统调用会返回0
    end = end + increment;                //此时更新之前记录的program break的位置
    return (void *)old;                   //并将旧program break的位置作为_sbrk()的返回值返回
  }
  return (void *)-1;                      //若该系统调用失败, _sbrk()会返回-1
}
