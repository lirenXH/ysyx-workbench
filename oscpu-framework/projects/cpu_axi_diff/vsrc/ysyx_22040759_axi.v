...
module ysyx_22040759_axi # (
    ...
    wire w_trans    = rw_req_i == `REQ_WRITE;  //1              判断读写请求
    wire r_trans    = rw_req_i == `REQ_READ;   //0   始终为1     
    wire w_valid    = rw_valid_i & w_trans;    //写有效  （特化valid）
    wire r_valid    = rw_valid_i & r_trans;    //读有效
